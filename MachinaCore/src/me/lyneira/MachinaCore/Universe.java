package me.lyneira.MachinaCore;


import me.lyneira.MachinaCore.machina.Machina;

/**
 * Represents all machinae in a world. This universal block store keeps track of
 * all blocks in the world that belong to a machina. It also prevents machinae
 * from intersecting each other and provides fast testing whether any block in
 * the world is part of a {@link Machina}.
 * 
 * @author Lyneira
 */
public class Universe {
    // The world belonging to this universe.
    
    // A list of all machinae in the world.
    
    // A fast map from block to its corresponding machina (or null if none)
}
